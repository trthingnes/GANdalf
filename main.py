...
NUM_EPOCHS = 50  # Numbers of iterations through training set
NOISE_DIMENSION = 50
# How many samples the data is split into for training, helps keep memory use down
BATCH_SIZE = 128
TRAIN_ON_GPU = True
# Makes sure that each training session gets stored in a unique folder
UNIQUE_RUN_ID = str(uuid.uuid4())
PRINT_STATS_AFTER_BATCH = 50  # How many batches to get through before printing stats
OPTIMIZER_LR = 0.0002
...
def save_models(generator, discriminator, epoch):
    ...
    torch.save(generator.state_dict(),
               f'./runs/{UNIQUE_RUN_ID}/generator_{epoch}.pth')
    torch.save(discriminator.state_dict(),
               f'./runs/{UNIQUE_RUN_ID}/discriminator_{epoch}.pth')
